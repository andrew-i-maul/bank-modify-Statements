...
def apply_mapping(row, mapping_rules, pattern_matches):
    ...
    # Now including Bank and Owner in the combined text
    combined_text = f"{row['Category']} {row['SubCategory']} {cleaned_description} {row['Owner']} {row['Bank']}"
...
def process_bank_statement(input_file, output_file, mapping_rules, pattern_matches):
    ...
    Returns:
        tuple: (DataFrame, total_records, changed_records) - Processed DataFrame and statistics
    """
    try:
    ...
    try:
        ...
        columns_order = ['Category', 'SubCategory', 'Description', 'Amount', 'Date', 'Bank', 'Owner', 'changed']
        df = df[columns_order]
            
        df.to_csv(output_file, index=False)
        logging.info(f"Processed data saved to {output_file}")
        return df, total_records, changed_records
    except Exception as e:
...
def process_directory(input_dir, output_dir, mapping_rules_file):
    ...
    Also creates a consolidated file containing all processed records.
    ...
    # List to store all processed DataFrames for consolidation
    all_processed_dfs = []
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith('.csv'):
            ...
            processed_df, total_records, changed_records = process_bank_statement(
    ...
    if all_processed_dfs:
        ...
        consolidated_output_path = os.path.join(output_dir, "Consolidated_Transactions.csv")
        
        # Remove 'changed' column if it exists
        if 'changed' in consolidated_df.columns:
            consolidated_df = consolidated_df.drop('changed', axis=1)
            
        # Sort by date - handle dd/mm/yyyy format
        ...
        logging.info(f"\nConsolidated file created: {consolidated_output_path}")
        logging.info(f"Total records in consolidated file: {len(consolidated_df)}")
