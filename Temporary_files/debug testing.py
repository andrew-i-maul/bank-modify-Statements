...
def apply_mapping(row, mapping_rules, pattern_matches):
    ...
    combined_text = f"{row['Bank']} {row['Owner']} {row['Category']} {row['SubCategory']} {cleaned_description}"
    
    # Apply mapping rules
    for pattern, rule_info in mapping_rules.items():
        if re.search('(?=.*Atm Withdrawal)(?=.*Rundle)(?=.*Andrew)', 'BankSA Andrew Withdrawals & Transfers ATM & EFTPOS Atm Withdrawal Rundle Place Os Adelaide Au'):
